...
class _FanHomePageState extends State<FanHomePage> {
  ...
  // HiveMQ Cloud
  final String host = "7fdb2149af904c77a46616484065e17f.s1.eu.hivemq.cloud";
  final int port = 8883; // TLS TCP port
  final String clientId = "flutter_fan_client";

  final String Username = "Thanh_Ban";
  ...
  Future<void> connectMQTT() async {
    // Tạo client TCP với TLS cho HiveMQ Cloud
    client = MqttServerClient(host, clientId);
    client.port = port; // 8883

    // Bật TLS – bắt buộc với HiveMQ Cloud
    client.secure = true;
    ...
    client.onDisconnected = () {
      print('=== ON DISCONNECTED CALLBACK ===');
      print('STATE: ${client.connectionStatus?.state}');
      print('RETURN CODE: ${client.connectionStatus?.returnCode}');
      print('DISCONNECT ORIGIN: ${client.connectionStatus?.disconnectionOrigin}');
    };
    client.onSubscribed = (topic) {
      print('=== SUBSCRIBED to $topic ===');
    };

    // Nếu muốn debug cert (KHÔNG nên dùng cho production)
    // client.onBadCertificate = (cert) {
    //   print('BAD CERTIFICATE: $cert');
    //   return true; // tạm chấp nhận mọi certificate
    // };
    ...
    // Message kết nối (giống sample của HiveMQ)
    final connMess = MqttConnectMessage()
        .withClientIdentifier(clientId)
        .startClean()                 // không dùng session cũ
        .keepAliveFor(20)
    ...
    try {
      ...
      print('AFTER CONNECT: '
          ...
          'RETURN=${client.connectionStatus?.returnCode}, '
          'ORIGIN=${client.connectionStatus?.disconnectionOrigin}');
      ...
      if (client.connectionStatus?.state == MqttConnectionState.connected) {
        ...
        // Không exception nhưng connect fail
        print('MQTT NOT CONNECTED, RETURN CODE: '
